...
abr = int(re.compile("(\d+)").search(aprobe.metadata['bitrate']).group(1) + '000')
asr = aprobe.streams[0].__getattribute__('sample_rate')

vprobe = FFProbe(video_filename)
vdec = vprobe.streams[0].codec_name
vbr = int(re.compile("(\d+)").search(vprobe.metadata['bitrate']).group(1) + '000')
...
if ERRCATCH == 0:
    ...
    for i in probef.streams:
        ...
        if i.codec_name == adec:
            ...
            adec=i.codec_name
    tbr =  int(re.compile("(\d+)").search(probef.__dict__['metadata']['bitrate']).group(1) + '000')
