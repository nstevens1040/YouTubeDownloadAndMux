...
def sync_execute_download(uri, filename):
    h = requests.head(uri)
    ...
    with requests.get(uri, stream=True) as r:
        with open(filename, 'wb') as f:
            with tqdm(total=file_size, unit='B',unit_scale=True,unit_divisor=1024,desc=name,initial=0,ascii=True,miniters=1) as pbar:
...
if not os.path.isdir(homefolder + "/Desktop/"):
    os.mkdir(homefolder + "/Desktop/")
...
if sys.platform == 'linux':
    sync_execute_download(a_selection['url'],audio_filename)
else:
    execute_download(a_selection['url'],audio_filename, asize)

print("audio download complete")
if sys.platform == 'linux':
    sync_execute_download(v_selection['url'],video_filename)
else:
...
abr = re.compile("(\d+)").search(aprobe.metadata['bitrate']).group(1) + '000'
asr = aprobe.streams[0].__getattribute__('sample_rate')

vprobe = FFProbe(video_filename)
vdec = vprobe.streams[0].codec_name
vbr = re.compile("(\d+)").search(vprobe.metadata['bitrate']).group(1) + '000'
...
if vdec == 'vp9':
    ...
    for i in list(range(0,(32 - len("    Intermediate video file: ")))):
        print(" ",end='')
    print(Fore.YELLOW + temp_video)
    ERRCATCH = 0
    try:
        ffmpeg.run(ffmpeg.output(ffmpeg.input(video_filename,vcodec=vdec),temp_video,video_bitrate=vbr,vcodec='libx264'),capture_stdout=True, capture_stderr=True, input=None, quiet=True, overwrite_output=True)
    except:
        ERRCATCH = 1
    if ERRCATCH == 0:
        os.remove(video_filename)
        video_filename = temp_video
        vprobe = FFProbe(video_filename)
        vdec = vprobe.streams[0].codec_name
        vbr = re.compile("(\d+)").search(vprobe.metadata['bitrate']).group(1) + '000'
        print(Fore.WHITE + "        Transcoding succeeded!")
...
if ERRCATCH == 0:
    ...
    for i in probef.streams:
        if i.codec_name == vdec:
            vbr=i.__dict__['bit_rate']
            vdec=i.codec_name
        if i.codec_name == adec:
            abr=i.__dict__['bit_rate']
            asr=i.__dict__['sample_rate']
            adec=i.codec_name
    tbr =  re.compile("(\d+)").search(probef.__dict__['metadata']['bitrate']).group(1) + '000'
